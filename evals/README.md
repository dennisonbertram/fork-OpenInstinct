...
```sh
pnpm eval:agent --max-cost-usd 1
```

Run one family while iterating:

```sh
pnpm eval:agent --max-cost-usd 1 --tag safety
pnpm eval:agent --max-cost-usd 1 --tag routing
```

Produce JUnit output for CI:

```sh
pnpm eval:ci --max-cost-usd 1
```
...
Real-model supervisors require an explicit `--max-cost-usd <USD>` before they
start. This is an operator-approved ceiling for the attempt series, not a
provider price lookup: no price service is consulted. One attempt is the
...
starts work. The manifest records exact Git SHA/dirty state, Node and Eve
versions, a content hash and paths for the case tree, wall-clock mode,
requested model, configured reasoning, judge, timeout, concurrency, all
...
Observed model IDs are the evidence of the concrete model actually selected;
...
rather than guessed. Costs similarly remain `unknown` unless every completed
model step reports `usage.costUsd`.

Use the normal workspace model resolver without changing product defaults:

```sh
pnpm eval:agent --max-cost-usd 1 --model openai/gpt-5.6-sol-fast --tag smoke
```
