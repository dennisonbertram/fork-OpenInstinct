...
export interface BrowserImageArtifactFile {
  readonly data: Buffer;
  readonly filename: string;
  readonly mimeType: string;
}

export async function prepareBrowserImageArtifactDelivery(
  message: string,
  input: {
  ...
  const loaded = await Promise.all(
    selected.map(async (reference) => ({
      image: await readBrowserImageArtifact(input.scope, reference.id, {
  ...
  const files = loaded.flatMap(({ image }) =>
    image
      ? [
          ...
          } satisfies BrowserImageArtifactFile,
        ]
      : []
  );
...
async function readBrowserImageArtifact(
  scope: AccessScope,
  artifactId: string,
