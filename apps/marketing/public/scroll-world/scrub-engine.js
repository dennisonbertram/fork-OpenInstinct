...
function mountScrollWorld(container, config) {
  ...
  // Embed mode: the world lives as a pinned section inside a normal page (e.g. between
  // a hero and the rest of the homepage) instead of owning the whole viewport. The
  // visual layers go in a position:sticky pin and the camera is driven by scroll
  // RELATIVE to this section. Standalone pages omit `embed` and keep the full-page path.
  const EMBED = !!config.embed;
  if (EMBED) container.classList.add("sw-embed");
  ...
  if (!EMBED && config.nav !== false) topbar.appendChild(nav);
  ...
  if (EMBED) {
    // A sticky pin holds every visual layer; the container's own height (set in layout())
    // provides the scroll length. The pin sticks to the top of the viewport while the
    // section scrolls past, so the camera scrubs in place, then releases into the next
    // page section. No separate `track` spacer in this mode.
    const pin = el("div", "sw-pin");
    [sky, scrollbar, topbar, stage, copylayer, route, hint].forEach((n) =>
      pin.appendChild(n)
    );
    container.appendChild(pin);
  } else {
    [sky, scrollbar, topbar, stage, copylayer, route, hint, track].forEach(
      (n) => container.appendChild(n)
    );
  }
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.eyebrow
        ? `<span class="sw-copy__eyebrow">${esc(s.eyebrow)}</span>`
        : "") +
  ...
  let baseTop = 0; // embed: document Y where this section's scroll begins
  ...
  function layout() {
    ...
    totalW = off;
    const scrollLen = totalW * vh + vh + "px"; // +1vh so the last flight completes
    if (EMBED) {
      container.style.height = scrollLen; // the container itself is the scroll region
      baseTop = Math.max(
        0,
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({
      top: baseTop + seg.start + (seg.end - seg.start) * 0.5,
  ...
  function read() {
    // In embed mode the camera is driven by scroll relative to this section, and the
    // sticky pin naturally shows only while the section is on screen (no viewport-fixed
    // layers bleeding over the rest of the page), so no manual visibility gating needed.
    const y = EMBED
      ? clamp((window.scrollY || window.pageYOffset) - baseTop, 0, totalW * vh)
      : window.scrollY || window.pageYOffset;
...
function seedParticles(host, reduce) {
  ...
  const seeds = [
    7, 23, 41, 58, 71, 88, 12, 34, 52, 66, 83, 95, 18, 29, 47, 63, 77, 91, 5,
    38, 55, 69, 82, 97,
  ];
  for (let k = 0; k < 20; k++) {
...
function injectCSS() {
  ...
  /* Embed mode: pin the visuals inside a sticky section instead of the viewport, so the
     world scrubs in place then releases into the rest of the page. */
  .sw-embed{display:block;position:relative;}
  .sw-embed .sw-pin{position:sticky;top:0;height:100vh;height:100dvh;overflow:hidden;}
  .sw-embed .sw-sky,.sw-embed .sw-stage,.sw-embed .sw-copylayer,.sw-embed .sw-route{position:absolute;}
  .sw-embed .sw-topbar,.sw-embed .sw-scrollbar,.sw-embed .sw-hint{display:none;}
...
if (typeof module !== "undefined" && module.exports)
  module.exports = { mountScrollWorld };
