...
| Split auth page  | `grid max-w-5xl items-end gap-10 lg:grid-cols-[minmax(0,28rem)_minmax(0,1fr)]`; `Card` with `w-full max-w-md gap-6 p-8 justify-self-center lg:justify-self-start`; mascot column `order-first flex justify-center lg:order-0 lg:justify-start`; the mascot is a static import next to the route, not a `public/` file | sign-in page                         |
...
- The sidebar header places the 36px clay Jory avatar in a muted circular
  background to the left of the colorful wordmark. Both are vertically centered
  in a large `SidebarMenuButton`, with a 12px gap.
