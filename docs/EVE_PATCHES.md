...
projection, never as the record of a task. The membership projection shares that
seam and the same property.

Membership was added on 2026-09-11 because the terminal-only projection could not
answer whether a cohort had finished: it omitted every entry without a
`terminalView`, so a root could not distinguish one of two tasks finishing from
the only task finishing. That is the case that must not produce a premature
summary.
