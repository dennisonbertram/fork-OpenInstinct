...
describe("Square eval artifact workflow", () => {
  ...
  it("dispatches exactly one explicitly budgeted paid eval target", () => {
    ...
    expect(workflow).toMatch(/if: \$\{\{ inputs\.run == 'square' \}\}[\s\S]*?run: pnpm eval:square --max-cost-usd 4 --estimated-cost-usd 4 --with-database --junit \.eve\/junit\.xml/);
    expect(workflow).toMatch(/if: \$\{\{ inputs\.run == 'agent-smoke' \}\}[\s\S]*?run: pnpm eval:agent --max-cost-usd 1 --estimated-cost-usd 1 --repetitions 1 --model openai\/gpt-5\.6-sol-fast --tag smoke --junit \.eve\/junit\.xml/);
